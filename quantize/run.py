    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            ...
            low_quantizer = LowQuantizer(
                subset[name].weight,
                itms = math.ceil(1.0/args.low_frac),
                method=args.low_quant_method,
                groupsize=args.groupsize,
            )
            high_quantizer = HighQuantizer( #HighQuantizer
                args.high_bit,
                True,
                False,
                False,
        ...
        handles = []
        for name in gpts: #name self.attn.q_proj
            handles.append(subset[name].register_forward_hook(add_batch(name)))
        inp_mean = inps.mean(0)
        for j in range(args.nsamples):
            outs[j] = layer(inps[j].unsqueeze(0), attention_mask=attention_mask)[0] #inps 128*2048*4096
        for h in handles:
            h.remove()
        ...
        for name in gpts:
            ...
            info = gpts[name].fasterquant(args.mask_type,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "model", type=str, help="model to load; for example `llama/llama-7b`."
    )
    parser.add_argument(
        "dataset",
        type=str,
    ...
    parser.add_argument(
        "low_quant_method",
        type=str,
        choices=["xnor", "braq", "sign", "no", "2bit", "4bit", "prune"],
