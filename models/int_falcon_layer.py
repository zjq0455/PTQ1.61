...
class QuantFalconDecoderLayer(nn.Module):
    def __init__(self, config: FalconConfig,
        ...
        if config.new_decoder_architecture:
            ...
            self.ln_attn = PTQLayerNorm(ori_layer.ln_attn)
            # The layer norm before the MLP
            self.ln_mlp =  PTQLayerNorm(ori_layer.ln_mlp)
        else:
            self.input_layernorm = PTQLayerNorm(ori_layer.input_layernorm)
            if not config.parallel_attn:
                self.post_attention_layernorm = PTQLayerNorm(ori_layer.post_attention_layernorm)
    ...
    def PTQ_parameters(self, use_shift=True):
        params = []
        template = "smooth" if use_shift else "smooth_scale"
        for n, m in self.named_parameters():
